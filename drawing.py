...
class Draw():
    def __init__(self, root):
        ...
        # Pick a color for drawing from color panel
        ...
        colors = ['blue', 'red', 'green', 'orange', 'violet', 'black', 'yellow', 'purple', 'pink', 'white', 'brown',
                  'indigo']
        i = j = 0
        for color in colors:
        ...
        self.save_btn = Button(self.root, text="ScreenShot", bd=4, bg='white', command=self.save_drawing, width=9,
                               relief=RIDGE)
        ...
        self.pointer_frame = LabelFrame(self.root, text='size', bd=5, bg='white', font=('arial', 15, 'bold'),
                                        relief=RIDGE)
        self.pointer_frame.place(x=0, y=350, height=200, width=70)
    ...
    def paint(self, event):
        if self.last_x and self.last_y:
            self.background.create_line(self.last_x, self.last_y, event.x, event.y, fill=self.pointer,
    ...
    # Function for choosing the color of the pointer
